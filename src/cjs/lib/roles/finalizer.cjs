...
class Finalizer extends signer_js_1.Signer {
  /**
   ...
   */
  prepareFinalization(inputIndex) {
   ...
   * @returns PreparedFinalization if successful, undefined if cannot finalize
   */
  prepareFinalizeTaproot(inputIndex) {
    // Key path spend - just need TAP_KEY_SIG
    const tapKeySignature = this.getInput(
      inputIndex,
      input_1.InputTypes.TAP_KEY_SIG,
    );
    if (tapKeySignature) {
      const serializedWitness = (0, witness_1.serializeWitnessStack)([
        tapKeySignature,
      ]);
      return {
        inputIndex,
        scriptType: types_js_1.SCRIPT_TYPE.P2TR,
   ...
   * @returns PreparedFinalization if successful, undefined if cannot finalize
   */
  prepareFinalizeWitness(inputIndex) {
    ...
    const { script } =
      input_1.InputField[input_1.InputTypes.WITNESS_UTXO].decode(
        witnessUtxoBuffer,
      );
    const scriptType = (0, script_1.detectScriptType)(script);
    const partialSignatures = this.getPartialSigs(inputIndex);
    const redeemScript = this.getInput(
      inputIndex,
      input_1.InputTypes.REDEEM_SCRIPT,
    );
    const witnessScript = this.getInput(
      inputIndex,
      input_1.InputTypes.WITNESS_SCRIPT,
    );
    // P2WPKH - single signature
    if (
      scriptType === types_js_1.SCRIPT_TYPE.P2WPKH &&
    ...
    if (
      scriptType === types_js_1.SCRIPT_TYPE.P2WSH &&
      witnessScript &&
      ...
      return {
        inputIndex,
        scriptType: types_js_1.SCRIPT_TYPE.P2WSH,
        finalScriptWitness: (0, witness_1.serializeWitnessStack)(
          witnessElements,
        ),
      };
    }
    // P2SH wrapping witness
    if (scriptType === types_js_1.SCRIPT_TYPE.P2SH && redeemScript) {
      const redeemScriptType = (0, script_1.detectScriptType)(redeemScript);
      // P2SH-P2WPKH
      if (
        redeemScriptType === types_js_1.SCRIPT_TYPE.P2WPKH &&
        partialSignatures.length === 1
      ) {
        const partialSignature = partialSignatures[0];
        // scriptSig = push(redeemScript)
        const finalScriptSig = (0, uint8array_tools_1.concat)([
          (0, encoding_1.encodeVarInt)(redeemScript.length),
          redeemScript,
        ]);
        ...
        return {
          inputIndex,
          scriptType: types_js_1.SCRIPT_TYPE.P2SH,
          finalScriptSig,
          finalScriptWitness: serializedWitness,
        };
      }
      // P2SH-P2WSH
      if (
        redeemScriptType === types_js_1.SCRIPT_TYPE.P2WSH &&
        witnessScript &&
        partialSignatures.length > 0
      ) {
        // scriptSig = push(redeemScript)
   ...
   * @returns PreparedFinalization if successful, undefined if cannot finalize
   */
  prepareFinalizeLegacy(inputIndex) {
    ...
    if (!redeemScript && partialSignatures.length === 1) {
      const partialSignature = partialSignatures[0];
      const finalScriptSig = (0, uint8array_tools_1.concat)([
        (0, encoding_1.encodeVarInt)(partialSignature.signature.length),
        partialSignature.signature,
        (0, encoding_1.encodeVarInt)(partialSignature.pubkey.length),
        partialSignature.pubkey,
      ]);
      return {
        inputIndex,
        scriptType: types_js_1.SCRIPT_TYPE.P2PKH,
        finalScriptSig,
      };
    }
    // P2SH (bare multisig): scriptSig = OP_0 <sigs...> <redeemScript>
    if (redeemScript && partialSignatures.length > 0) {
      const scriptSigParts = [new Uint8Array([script_1.OPS.OP_0])]; // CHECKMULTISIG bug
      ...
      scriptSigParts.push((0, encoding_1.encodeVarInt)(redeemScript.length));
      scriptSigParts.push(redeemScript);
      return {
        inputIndex,
        scriptType: types_js_1.SCRIPT_TYPE.P2SH,
        finalScriptSig: (0, uint8array_tools_1.concat)(scriptSigParts),
      };
    }
    return undefined;
  }
  ...
  cleanupInput(inputIndex) {
    ...
    for (const keyHex of [...this.inputMaps[inputIndex].keys()]) {
      const { type, data } = (0, psbtkey_1.parseKey)(keyHex);
   ...
   * @param inputIndex - Index of the input
   */
  getFinalScriptSig(inputIndex) {
    return this.getInput(inputIndex, input_1.InputTypes.FINAL_SCRIPTSIG);
  }
  /**
   * Get the final scriptWitness for an input
   * @param inputIndex - Index of the input
   */
  getFinalScriptWitness(inputIndex) {
    return this.getInput(inputIndex, input_1.InputTypes.FINAL_SCRIPTWITNESS);
  }
  /**
   * Get the final witness stack for an input (deserialized)
   * @param inputIndex - Index of the input
   */
  getFinalWitnessStack(inputIndex) {
    const witnessBuffer = this.getInput(
      inputIndex,
      input_1.InputTypes.FINAL_SCRIPTWITNESS,
    );
    if (!witnessBuffer) return undefined;
    return input_1.InputField[input_1.InputTypes.FINAL_SCRIPTWITNESS].decode(
      witnessBuffer,
    );
  }
  /**
   ...
   * @param inputIndex - Index of the input
   */
  getTapLeafScripts(inputIndex) {
    const entries = this.getInputsOfType(
      inputIndex,
      input_1.InputTypes.TAP_LEAF_SCRIPT,
    );
    const field = input_1.InputField[input_1.InputTypes.TAP_LEAF_SCRIPT];
